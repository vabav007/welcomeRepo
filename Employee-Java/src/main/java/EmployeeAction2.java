...
public class EmployeeAction2 {
    ...
    public static int getTotalEmployeesByDepartment(){
       return getEmployees2().stream()
                ...
                .distinct().reduce(Integer::sum).ifPresent(System.out::println); // type error
    }
    ...
    public static Map<String, Employee> getEmployeeAverageSalary() {
        Map<String, Employee> average =
                getEmployees2().stream()
                        .collect(groupingBy(
                                e -> e.getDepartment(),
                                collectingAndThen(averagingDouble(Employee::getSalary), Collections::unmodifiableList) ) // type error
                        );
        return average;
    }
